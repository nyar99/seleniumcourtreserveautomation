...
```bash
argh$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
argh$ sam local start-api
argh$ curl http://localhost:3000/
```
...
```bash
argh$ sam logs -n HelloWorldFunction --stack-name argh --tail
```
...
```bash
argh$ pip install pytest pytest-mock --user
argh$ python -m pytest tests/ -v
```
...
```bash
aws cloudformation delete-stack --stack-name argh
```
